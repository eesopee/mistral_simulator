...
def radec_cross_scan(target, span, t0, duration, include_overhead=False, loc=None):
...
def daisy_scan(target, radius, t0, duration, tau_radial, loc):
    ...
    return t/86400, x,y,coords_new.alt.value,coords_new.az.value
...
if __name__ == '__main__':
    ...
    loc = EarthLocation.of_site(args.site) #retrieving lon-lat of SRT to calculate AZ EL and vice versa
    discos_sampling_frequency = 10
 
    start = time.time()
 
    target = SkyCoord.from_name(args.target, frame='icrs')
    t0 = astropy.time.Time(float(args.t0), format="unix").mjd #59200.84973159712
    
    radius = float(args.radius)
    radius /= 60
 
    duration = float(args.duration) / 86400 #scan duration in days
    
    if args.scan == "daisy":    
        print("Daisy")
        t,ra,dec,alt,az = daisy_scan(target=target, t0=t0, duration=duration, tau_radial=duration/22, radius=radius, loc=loc)
    
    if args.scan == "cross_radec":
        print("radec")
        t,ra,dec,alt,az = radec_cross_scan(target, span=radius, t0=t0, duration=duration, loc=loc)
    ...
    discos_hdul.writeto(args.outname+'_{:f}.fits'.format(start), overwrite=True)
